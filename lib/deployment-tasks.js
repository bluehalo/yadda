...
var secretTools = require('./secret-tasks');
 ...
 * @param  {module:AWSTools~Registry} registry - Remove ECR Registry to pull from
...
function buildImages(registry) {
	...
	return function(buildOptions) {
		...
		for(let name in buildOptions.images)
			if(buildOptions.images.hasOwnProperty(name))
				if(buildOptions.images[name].repo == null)
					delete buildOptions.images[name];
...
function createTaskDefinition(taskInfo, taskId, options) {
	...
	if(options.DeploymentCenter.secret)
		taskDefinition.environment = _.merge(secretTools.getEnvironmentVariables(options), taskDefinition.environment);
...
exports.deploy = function(options) {
	...
	.then(pruneTasks())
	.then(buildImages(registry))
	.then(pushImagesToECR(registry))
...
exports.pull = function(options) {
	...
	return Q.when(options)
		.then(setupDeploymentCenter())
		.then(getCurrentDeployment())
		.then(pullImages(containerService, registry));
};
