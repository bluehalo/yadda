...
exports.refreshSecrets = function(options){
	return Q.when(options)
		.then(function(){
			const secrets = options.DeploymentCenter.secret;

			if(!secrets.cacheBusterKey){
				return logger.warn('no cache buster key set');
			}

			const tableDetails = getCredentialTableDetails(options);
			const dynamoDB = new AWS.DynamoDB.DocumentClient({
				region: tableDetails.region
			});
			const params = {
				TableName: tableDetails.name,
				Item: {
					name: secrets._prefixKey(secrets.cacheBusterKey),
					version: _.padStart('0', 19, '0'),
					contents: Date.now()
				}
			};
			return Q.ninvoke(dynamoDB, 'put', params)
				.then(function(){
					logger.info('Successfully refreshed secrets');
				})
				.fail(function(err){
					logger.error('Error encountered refreshing the secrets:', err);
				});
		});
};
